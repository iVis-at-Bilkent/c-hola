...
const cholaConstants = require('./cholaConstants');
const cholaGraphManager = require('./cholaGraphManager');
const cholaNode = require('./cholaNode');
const cholaEdge = require('./cholaEdge');
...
cholaLayout.prototype.convertToClusteredGraph = function(nodes, edges, clusters){
    ...
    for(let i = 0; i < nodes.length; i++) {
        ...
        if(!clustered){
            ...
            let cholaNode = this.newNode(new PointD(cytoNode.position('x') - dimensions.w / 2,
                cytoNode.position('y') - dimensions.h / 2),
                new DimensionD(parseFloat(dimensions.w), parseFloat(dimensions.h)));
            cholaNode.setClusterId(-1);
            cholaNode.setId( nodes[i].data('id') );
            rootGraph.add(cholaNode);
    ...
    for(let i = 0; i < edges.length; i++) {
        ...
        let cholaEdge = self.newEdge(sourceNode, targetNode, null);
        ...
        if(sourceClusterID === targetClusterID && sourceClusterID !== -1 && targetClusterID !== -1){
            cholaEdge.isIntraCluster = true;
            cholaEdge.getSource().getOwner().add(cholaEdge, cholaEdge.getSource(), cholaEdge.getTarget());
        }
        else{
            cholaEdge.isIntraCluster = false;
            this.graphManager.add(cholaEdge, cholaEdge.getSource(), cholaEdge.getTarget());
        }
    }
...
cholaLayout.prototype.doStep1 = function(){
    ...
    for(let i = 0; i < allGraphs.length; i++){
        ...
        if(graph instanceof cholaCircle) {
            ...
            for (let i = 0; i < clusteredNodes.length; i++) {
                let cholaOnCircleNode = clusteredNodes[i];

                let avsdfNode = avsdfLayout.newNode(null);
                let loc = cholaOnCircleNode.getLocation();
            ...
            for(let i = 0; i < allEdges.length; i++) {
                ...
                if(clusteredNodes.includes( edge.getSource() ) && clusteredNodes.includes( edge.getTarget() )){
                    let avsdfSource = cholaToAvsdf.get( edge.getSource() );
                    let avsdfTarget = cholaToAvsdf.get( edge.getTarget() );
            ...
            for(let i = 0; i < clusteredNodes.length; i++){
                let cholaOnCircleNode = clusteredNodes[i];
                let avsdfNode = cholaToAvsdf.get(cholaOnCircleNode);
                let loc = avsdfNode.getLocation();
                cholaOnCircleNode.setLocation(loc.x, loc.y);
                cholaOnCircleNode.getOnCircleNodeExt().setIndex(avsdfNode.getIndex());
                cholaOnCircleNode.getOnCircleNodeExt().setAngle(avsdfNode.getAngle());
            }
            ...
            {
                let parentchola = graph.getParent();
...
cholaLayout.prototype.doStep2 = function(){
    ...
    for (let i = 0; i < nonOnCircleNodes.length; i++){
        let cholaNode = nonOnCircleNodes[i];
    ...
    for (let i = 0; i < allEdges.length; i++ ){
        let cholaEdge = allEdges[i];
        let sourcechola = cholaEdge.getSource();
        let targetchola = cholaEdge.getTarget();
        ...
        if (sourcechola.getOnCircleNodeExt() != null){
            ...
            sourcechola = cholaEdge.getSource().getOwner().getParent();
        }
        if (targetchola.getOnCircleNodeExt() != null){
            // Target node is an on-circle node, take its parent as target node
            targetchola = cholaEdge.getTarget().getOwner().getParent();
        }

        let sourceCose = cholaNodeToCoseNode.get(sourcechola);
        let targetCose = cholaNodeToCoseNode.get(targetchola);
        ...
        if (sourceIndex !== targetIndex){
            ...
            coseEdgeTocholaEdges.get(newEdge).push(cholaEdge);
        }
    }
    ...
    {
        let cholaNode = onCircleNodes[i];
        let loc = cholaNode.getLocation();
        let parentLoc = cholaNode.getOwner().getParent().getLocation();
        cholaNode.setLocation(loc.x + parentLoc.x, loc.y + parentLoc.y);
    }
...
cholaLayout.prototype.runSpringEmbedderTick = function (){
    ...
    if ( (this.iterations % cholaConstants.CONVERGENCE_CHECK_PERIOD) === 0)
    {
        ...
        let notTooEarly = this.step !== cholaLayout.STEP_4 || this.iterations > this.maxIterations / 4;
    ...
    else if (this.step === cholaLayout.STEP_4)
    {
        ...
        let iterationInPeriod = this.iterations % cholaConstants.SWAP_PERIOD;
...
cholaLayout.prototype.calcRepulsionForces = function() {
    ...
    for (let i = 0; i < inCircleNodes.length; i++) {
        ...
        for(let i = 0; i < childNodes.length; i++){
            let childcholaNode = childNodes[i];
...
cholaLayout.prototype.moveNodes = function(){
    if (this.phase !== cholaLayout.PHASE_PERFORM_SWAP)
    {
        ...
        // energy.
        let cholaOnCircleNodes = this.graphManager.getOnCircleNodes();
        let size = cholaOnCircleNodes.length;
        ...
        for (let i = 0; i < size; i++) {
            firstNode = cholaOnCircleNodes[i];
            ...
            let pair = new cholaOnCircleNodePair(firstNode,
                secondNode,
                discrepancy,
                inSameDirection);
            ...
            while (lookForSwap && nonSafePairs.length > 0)
            {
                ...
                if (!rollback && int2 === int1)
                {
                    rollback =
                        ...
                        cholaConstants.MIN_DISPLACEMENT_FOR_SWAP;
                }
...
cholaLayout.prototype.findAndMoveInnerNodes = function (){
    ...
    for (let i = 0; i < graphs.length; i++){
        let cholaCircle = graphs[i];

        // Count inner nodes not to exceed user defined maximum
        let innerNodeCount = 0;

        if (cholaCircle !== this.getGraphManager().getRoot())
        {
            // It is a user parameter, retrieve it.
            let maxInnerNodes = cholaCircle.getNodes().length * this.maxRatioOfNodesInsideCircle;

            // Look for an inner node and move it inside
            let innerNode = this.findInnerNode(cholaCircle);
            ...
            while (innerNode !== null && innerNode !== undefined && innerNodeCount < maxInnerNodes)
            {
                ...
                {
                    innerNode = this.findInnerNode(cholaCircle);
                }
            }
        }
    }
...
cholaLayout.prototype.findInnerNode = function (cholaCircle){
    let innerNode = null;
    let onCircleNodeCount = cholaCircle.getOnCircleNodes().length;

    // First sort the nodes in the circle according to their degrees.
    let sortedNodes = cholaCircle.getOnCircleNodes();
        ...
        for(let i = 0; i < circleSegment.length; i++) {
            ...
            for (let j = 0; j < neighbors.length; j++) {
                ...
                if (neighborOfSpanningNode !== candidateNode &&
                    neighborOfSpanningNode.getOwner() === cholaCircle &&
...
{
    let cholaCircle = innerNode.getOwner();
